...
def equal(u,v):
    ...
    assert u.D == v.D
    for i in u.D:
        if (i not in u.f) and (i in v.f) and v.f[i] != 0:
            return False
        elif (i in u.f) and (i not in v.f) and u.f[i] != 0:
            return False
        elif (i in u.f) and (i in v.f):
            if u.f[i] != v.f[i]:
                return False
    return True
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    diccionario = {d: u[d] + v[d] for d in u.D}
    vector = Vec(u.D, diccionario)
    return vector
...
def scalar_mul(v, alpha):
    ...
    True
    """
    dicc = {d: v[d] * alpha for d in v.D}

    vector1 = Vec(v.D, dicc)
